...
SNIPPET_CHARS = 240
# 生成带着父块上下文，首 token 经常超过改写用的 60s。超时走上游错误；连接若已先被掐，
# 前端只会看到「生成中断」。
...
async def generate(state: QAState, config: RunnableConfig) -> dict:
    ...
    deps = deps_of(config)
    timeout = max(
        getattr(settings, "llm_timeout_seconds", GENERATE_TIMEOUT_SECONDS),
    ...
    context_chars = sum(len(context.content) for context in contexts)
    logger.info("开始流式生成 context_chars=%s timeout=%s", context_chars, timeout)
    if writer is not None:
        # 鉴权后到首 token 常有一段空 chunk；先推一帧让 WS 层不要静默
        writer({"type": "heartbeat"})
    ...
    try:
        ...
        async with aclosing(
            ...
            async for chunk in stream:
                ...
                now = perf_counter()
                if writer is not None and now - last_beat >= 10:
                    writer({"type": "heartbeat"})
                    last_beat = now
                ...
                if not first_token_logged:
                    logger.info("生成首 token 耗时 %.2fs", now - started)
