...
| 发 | `status` / `token` / `citation` / `acl_notice` / `done` / `error` / `pong` |
...
3. **一次只处理一个 `ask`**。收到新 `ask` 前的循环是阻塞的，第二个问题会等前一个答完。
   并发处理会让两轮 token 交错写进同一个气泡，前端无法还原。
        ...
        async with aclosing(
            ...
            async for mode, payload in stream:
                if mode == "custom":
                    ...
                    continue
                await _forward_updates(websocket, payload, turn)
...
async def _forward_updates(websocket: WebSocket, payload: Any, turn: _Turn) -> None:
