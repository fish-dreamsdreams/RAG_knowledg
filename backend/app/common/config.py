...
class Settings(BaseSettings):
    """从环境变量与 .env 加载的运行时配置；字段名对应大写环境变量。"""
    ...
    # 本地模型
    embed_model: str = "E:/ai_models/modelscope_cache/models/BAAI/bge-m3"
    embed_device: str = "cuda"
    embed_sparse_enabled: bool = True
    # 启动即加载权重并跑一次最小编码：把权重加载与 CUDA context 创建挑到启动期，
    # 避免首个导入任务既等模型又等首次初始化
    embed_warmup_enabled: bool = True
    rerank_model: str = "E:/ai_models/modelscope_cache/models/BAAI/bge-reranker-v2-m3"
